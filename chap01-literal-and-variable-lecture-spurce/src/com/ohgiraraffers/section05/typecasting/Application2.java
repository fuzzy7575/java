package com.ohgiraraffers.section05.typecasting;
...
//        int inum = lnum;   //데이터 손실 가능성을 컴파일러가 알려준다. (에러남)
        int inum = (int) lnum;   //변경하려는 자료형을 명시하여 강게 형변환을 해야 함.
...
//        float fnum = dnum;   //데이터 손실 가능성을 컴파일러가 알려준다. (에러남)
        ...
        float fnum2 = 4.0f;
//        long lnum2 = fnum2;   //float는 4byte, long은 8byte임에도 자동 형변환 불가능(소수점 이하 데이터 손실)
        ...
        short snum2 = (short) ch;   //같은 2byte지만 부호비트(sign bit)로 인한 값의 범위가 다르기 때문이다.
